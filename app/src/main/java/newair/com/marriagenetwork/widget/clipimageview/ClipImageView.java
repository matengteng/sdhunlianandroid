 ...
 */
public class ClipImageView extends ImageView implements OnTouchListener {
	...
	private static final int BORDER_DISTANCE = ClipView.BORDER_DISTANCE;
	...
	protected void onDraw(Canvas canvas) {
		...
		borderlengthW = (int) (getWidth() - BORDER_DISTANCE *2);
		borderlengthH=borderlengthW*ClipView.lineHeight/ClipView.lineWidth;
	...
	public Bitmap clip(){
		...
		draw(canvas);
		//凑整，防止出现黑边
		return Bitmap.createBitmap(bitmap, (int) Math.ceil((getWidth() - borderlengthW) / 2f), (int) Math.ceil((getHeight() - borderlengthH) / 2f), borderlengthW, borderlengthH);
	}
