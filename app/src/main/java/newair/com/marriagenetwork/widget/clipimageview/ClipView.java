...
import newair.com.marriagenetwork.utils.ScreenUtils;

/**
 * 裁剪框
 * @author tianbin
 *
 * Created on 2014-11-27 下午3:49:20
 */
public class ClipView extends View {
	 ...
	 * 边框距左右边界距离，用于调整边框长度
	 */
	public static int BORDER_DISTANCE = 300;
	...
	public ClipView(Context context, AttributeSet attrs, int defStyle) {
		...
		BORDER_DISTANCE = ScreenUtils.getScreenWidth(context)/4;
		lineWidth = DEFAULT_BORDER_WIDTH;
		lineHeight = DEFAULT_BORDER_HEIGHT;
		lineColor = DEFAULT_BORDER_COLOR;
		isRound= true;
	...
	protected void onDraw(Canvas canvas) {
		...
		if(isRound){
			...
			// left
			canvas.drawRect(0, (height - borderlengthH) / 2, BORDER_DISTANCE,
					(height + borderlengthH) / 2, mPaint);
			// right
			canvas.drawRect(borderlengthW + BORDER_DISTANCE, (height - borderlengthH) / 2, width,
					(height + borderlengthH) / 2, mPaint);
			// 以下绘制边框线
		...
		//或者下面这种方式
		// top
//		canvas.drawLine(BORDERDISTANCE, (height - borderlengthW) / 2, width - BORDERDISTANCE, (height - borderlengthW) / 2, mPaint);
//		// bottom
//		canvas.drawLine(BORDERDISTANCE, (height + borderlengthW) / 2, width - BORDERDISTANCE, (height + borderlengthW) / 2, mPaint);
//		// left
//		canvas.drawLine(BORDERDISTANCE, (height - borderlengthW) / 2, BORDERDISTANCE, (height + borderlengthW) / 2, mPaint);
//		// right
//		canvas.drawLine(width - BORDERDISTANCE, (height - borderlengthW) / 2, width - BORDERDISTANCE, (height + borderlengthW) / 2, mPaint);
	}
